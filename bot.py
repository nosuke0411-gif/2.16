...
@app_commands.describe(minutes="探検時間（5, 15, 30, 60）", location="探検場所")
async def explore(interaction: discord.Interaction, minutes: Literal[5, 15, 30, 60], location: Literal["森", "山", "川辺", "廃墟", "神社"]):
    ...
    msg = f"{EXPLORE_LOCATIONS[location]['emoji']} **{location}** へ {minutes}分の探検に出発したよ！\n⏳ 終了予定: <t:{int(end_time.timestamp())}:R>"
...
async def combine_items(interaction: discord.Interaction, target: Literal["特製おやつ", "金のコイン", "祈りの結晶", "神秘の面"]):
    await interaction.response.defer(thinking=True)
    user_id = str(interaction.user.id)
    items = get_user_items(user_id)
